...
static int deadman = 300;

static int
usage(const char *prog) {
	fprintf(stderr, "%s <-c conffile> [-t <seconds>] [-d]\n\n", prog);
  fprintf(stderr, "\t-c conffile\tthe configuration file to load\n");
  fprintf(stderr, "\t-t\t\tset deadman timer (default: 300s)\n");
...
parse_cli_args(int argc, char * const *argv) {
  int c;
  while((c = getopt(argc, argv, "c:dt:")) != EOF) {
    switch(c) {
      case 'c':
        config_file = optarg;
        break;
      case 'd': debug = 1; break;
      case 't': deadman = atoi(optarg); break;
    }
  }
}
...
static int
deadman_kill(eventer_t e, int mask, void *closure, struct timeval *now) {
  mtevL(mtev_error, "Deadman timer triggered.\n");
  exit(2);
}
static int
child_main() {
  ...
  if(deadman) eventer_add_in_s_us(deadman_kill, NULL, deadman, 0);
