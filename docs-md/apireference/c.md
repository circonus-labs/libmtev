...
```c
int
mtev_ex_log(mtev_log_stream_t ls, const struct timeval *, const char *file, int line,
...
```c
int
mtev_ex_vlog(mtev_log_stream_t ls, const struct timeval *, const char *file, int line,
...
```c
int
mtev_log(mtev_log_stream_t ls, const struct timeval *, const char *file, int line,
         const char *format, ...)
```
...
>Given a flatbuffer serializatio of a log line, convert it to the flatcc type.
...
```
  ...
  * `buff` a pointer to memory containing the flatbuffer serialization.
...
>Instruct the logging system to use asynchronous logging for higher peroformance.
...
>Set or replace a key-value propoerty on a log stream
...
```c
int
mtev_vlog(mtev_log_stream_t ls, const struct timeval *, const char *file, int line,
          const char *format, ...)
```
