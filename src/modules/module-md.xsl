...
<xsl:param name="example" select="1"/>
<xsl:template match="/">
...
<xsl:for-each select="module/description/*">
  ...
  * **image**: <xsl:value-of select="module/image"/> </xsl:if>
  <xsl:if test="module/object">
  * **object**: <xsl:value-of select="module/object"/> </xsl:if>
...
<xsl:for-each select="module/examples/example">#### <xsl:value-of disable-output-escaping="yes" select="title"/>

```<xsl:value-of select="programlisting/@language"/>
<xsl:value-of disable-output-escaping="yes" select="programlisting"/>
```
