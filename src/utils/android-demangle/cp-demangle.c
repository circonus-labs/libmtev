/* Demangler for g++ V3 ABI.
   Copyright 2017 Circonus, Inc.
   Tiny parts written by Circonus.
...
void
cplus_demangle_set_buf(char *buff, size_t len) {
  no_alloc.buff = buff;
  no_alloc.len = len;
}

static inline void
d_growable_string_free (struct d_growable_string *dgs)
{
  if(dgs->buf != no_alloc.buff) free(dgs->buf);
}
static inline void
d_growable_string_resize (struct d_growable_string *dgs, size_t need)
{
  ...
  char *newbuf;
  if(no_alloc.buff) {
    if (need > no_alloc.len) {
      dgs->allocation_failure = 1;
      return;
    }
    dgs->buf = no_alloc.buff;
    dgs->alc = no_alloc.len;
    return;
  }
  ...
  if (output_buffer == NULL)
	...
	*length = alc;
      ...
      if (strlen (demangled) < *length)
	{
	  ...
	  if(demangled != no_alloc.buff) free (demangled);
